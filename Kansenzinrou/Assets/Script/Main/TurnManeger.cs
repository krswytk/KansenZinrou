            ...
            switch (stage)
            {
                case Config.Stage.待機:
                    ...
                    if(TSW){
                        ...
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "ターン目開始", false);
                    }
                ...
                case Config.Stage.表更新:               
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "表を更新", false);
                        //表更新
                        UIM.TableUpdate();
                    }
                    if ((Timer - NowTime) > (int)Config.FirstNumber.NextStage)//一定時間経過したら
                ...
                case Config.Stage.情勢イベント表示://ここでガチャ+効果の反映を行う               
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "情勢イベント設定", false);
                        //情勢イベント表示
                        if (Turn == 1)
                        {
                ...
                case Config.Stage.感染状態表示:            
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "感染状態表示", false);
                        //感染状態表示
                ...
                case Config.Stage.給料配布:
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "給料配布", false);
                        //給料配布
                        IA.Salary();
                    }
                    if ((Timer - NowTime) > (int)Config.FirstNumber.NextStage)//一定時間経過したら
                ...
                case Config.Stage.仕入れを0にする:
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "仕入れ値リセット", false);
                        //仕入れを0にする
                        IA.PurchasingRiset();
                    }
                    if ((Timer - NowTime) > (int)Config.FirstNumber.NextStage)//一定時間経過したら
                ...
                case Config.Stage.取引:
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "取引開始", false);
                        BusinessSW = true;
                    }
                ...
                case Config.Stage.情勢イベント削除:
                    ...
                    if (TSW)
                    {
                        ...
                        IventAll.LogOut(Turn + "情勢イベント削除", false);
                        IA.NewsOFF();
                    }
                    if ((Timer - NowTime) > (int)Config.FirstNumber.NextStage)//一定時間経過したら
                    {
                ...
                case Config.Stage.全ターン終了:
                    ...
                    if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;
                        IventAll.LogOut(Turn + "全行程終了", false);
                ...
                case Config.Stage.判定:
                    ...
                    ///if (TSW)
                    {
                        TSW = false;//一回のみ呼び出す処理
                        NowTime = Timer;                    
                        IventAll.LogOut(Turn + "敗北勝利判定", false);
